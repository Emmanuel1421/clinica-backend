...
export async function createPatient(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (typeof phone !== 'string' || !isValidPhone(phone)) {
      res.status(400).json({ success: false, message: 'Dados inválidos.', errors: { phone: 'Telefone é obrigatório (10 dígitos para fixo ou 11 para celular com 9).' } });
      return;
    }
    ...
    if (err instanceof Error && (err.message.includes('CPF') || err.message.includes('Telefone'))) {
...
export async function updatePatient(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (phone !== undefined && (typeof phone !== 'string' || !isValidPhone(phone))) {
      res.status(400).json({ success: false, message: 'Dados inválidos.', errors: { phone: 'Telefone é obrigatório (10 dígitos para fixo ou 11 para celular com 9).' } });
      return;
    }
    if (email !== undefined && (typeof email !== 'string' || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 254)) {
